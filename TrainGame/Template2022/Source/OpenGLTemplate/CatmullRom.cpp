...
void CCatmullRom::SetControlPoints()
{
	...
	m_controlPoints.push_back(glm::vec3(200, 0.1, 0));
	//m_controlPoints.push_back(glm::vec3(71, 0.1, 71));
	m_controlPoints.push_back(glm::vec3(0, 0.1, 200));
	//m_controlPoints.push_back(glm::vec3(-71, 0.1, 71));
	m_controlPoints.push_back(glm::vec3(-200, 0.1, 0));
	//m_controlPoints.push_back(glm::vec3(-71, 0.1, -71));
	m_controlPoints.push_back(glm::vec3(0, 0.1, -200));
...
void CCatmullRom::CreateCentreline()
{
	...
	// Creating a VAO called m_vaoCentreline and a VBO to get the points onto the graphics card
...
void CCatmullRom::CreateOffsetCurves()
{
	...
	for (unsigned int i = 0; i < m_centrelinePoints.size(); i++) {
		...
		glm::vec3 pNext = m_centrelinePoints[(i + 1)%(m_centrelinePoints.size()-1)];
	...
	vboLeft.AddData(&m_leftOffsetPoints[0], sizeof(glm::vec3));
	vboLeft.AddData(&texCoord, sizeof(glm::vec2));
	vboLeft.AddData(&normal, sizeof(glm::vec3));
	...
	vboRight.AddData(&m_rightOffsetPoints[0], sizeof(glm::vec3));
	vboRight.AddData(&texCoord, sizeof(glm::vec2));
	vboRight.AddData(&normal, sizeof(glm::vec3));
	// Upload the VBO to the GPU
...
void CCatmullRom::CreateTrack(string filename)
{
	...
	glm::vec2 TopRighttexCoord(1.0f, 0.0f);
	...
	for (unsigned int i = 0; i < m_centrelinePoints.size(); i++) {
		...
		if(i%2 == 0) 
			vbo.AddData(&BottomLefttexCoord, sizeof(glm::vec2));
		else
			vbo.AddData(&TopLefttexCoord, sizeof(glm::vec2));
	    vbo.AddData(&normal, sizeof(glm::vec3));

		vbo.AddData(&m_rightOffsetPoints[i], sizeof(glm::vec3));
		if(i%2 == 0)
			vbo.AddData(&BottomRighttexCoord, sizeof(glm::vec2));
		else
			vbo.AddData(&TopRighttexCoord, sizeof(glm::vec2));
		vbo.AddData(&normal, sizeof(glm::vec3));
	}
...
void CCatmullRom::RenderCentreline()
{
	...
	glLineWidth(5.0f);
	glBindVertexArray(m_vaoCentreline);
	glDrawArrays(GL_LINE_STRIP, 0, m_centrelinePoints.size()+1);
}
...
void CCatmullRom::RenderTrack()
{
	...
	glBindVertexArray(m_vaoTrack);
	m_texture.Bind();
	glDrawArrays(GL_TRIANGLE_STRIP, 0, m_rightOffsetPoints.size() + m_leftOffsetPoints.size() + 2);
}
